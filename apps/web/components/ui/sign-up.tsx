    ...
    >
      <defs>
        <linearGradient id="rev_grad1" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" style={{ stopColor: "var(--color-primary, #10b981)", stopOpacity: 0.8 }} />
          <stop offset="100%" style={{ stopColor: "var(--color-chart-3, #065f46)", stopOpacity: 0.6 }} />
        </linearGradient>
        <linearGradient id="rev_grad2" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" style={{ stopColor: "var(--color-chart-4, #6ee7b7)", stopOpacity: 0.9 }} />
          <stop offset="50%" style={{ stopColor: "var(--color-secondary, #059669)", stopOpacity: 0.7 }} />
          <stop offset="100%" style={{ stopColor: "var(--color-chart-1, #10b981)", stopOpacity: 0.6 }} />
        </linearGradient>
        <radialGradient id="rev_grad3" cx="50%" cy="50%" r="50%">
          <stop offset="0%" style={{ stopColor: "var(--color-destructive, #f43f5e)", stopOpacity: 0.8 }} />
          <stop offset="100%" style={{ stopColor: "var(--color-chart-5, #a7f3d0)", stopOpacity: 0.4 }} />
        </radialGradient>
...
const modalSteps = [
  ...
  { message: "Securing session...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
...
export interface AuthComponentProps {
  ...
  onEmailSignUp?: (email: string, password: string) => Promise<{ error?: string }>;
  onSuccess?: () => void;
...
export const AuthComponent = ({
  ...
  const executeAuth = async () => {
    ...
    try {
      ...
      if (mode === "signup") {
        if (onEmailSignUp) {
          res = await onEmailSignUp(email, password);
        } else {
        ...
        if (onEmailSignIn) {
          res = await onEmailSignIn(email, password);
        } else {
      ...
      if (res.error) {
        const msg =
          res.error.message ||
          (typeof res.error === "string" ? res.error : "Authentication failed. Please check your credentials.");
      ...
      }, Math.max(800, totalDuration / 2));
    } catch (err: any) {
      setModalErrorMessage(err?.message || "An unexpected error occurred.");
      setModalStatus("error");
      setIsSubmitting(false);
    }
  };
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (modalStatus !== "closed" || isSubmitting) return;
  ...
  const handleProgressStep = () => {
    if (authStep === "email") {
      ...
      if (mode === "signup") {
        ...
        if (isPasswordValid) executeAuth();
      }
    }
  };
  ...
  const defaultTitle = mode === "signup" ? "Get started with Us" : "Welcome back";
  const defaultSubtitle = mode === "signup" ? "Create an account to continue" : "Sign in to your account";
      ...
      {modalStatus !== "closed" && (
        ...
        >
          <motion.div
            ...
            className="relative bg-card/90 border-2 border-border shadow-2xl rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2"
          >
  ...
  return (
    <div className="bg-background min-h-screen w-screen flex flex-col">
      ...
      <div className={cn("flex w-full flex-1 h-full items-center justify-center bg-card", "relative overflow-hidden min-h-screen py-16")}>
        ...
        >
                      ...
                      <BlurFade key="password-field" className="w-full">
                            ...
                            <div className="glass-input">
                              ...
                              <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                {isPasswordValid ? (
                                  <button
                                    ...
                                    className="text-foreground/80 hover:text-foreground transition-colors p-2 rounded-full cursor-pointer"
                                  >
                              ...
                              >
                                <GlassButton
                                  ...
                                  onClick={handleProgressStep}
                                  size="icon"
                                  aria-label={mode === "signin" ? "Sign in" : "Submit password"}
                                  contentClassName="text-foreground/80 hover:text-foreground"
                                >
                                  <ArrowRight className="w-5 h-5" />
                                </GlassButton>
                        ...
                        <BlurFade inView delay={0.2}>
                          <button
                            ...
                            className="mt-4 flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground transition-colors cursor-pointer"
                          >
          ...
          {allowModeToggle && (
            <div className="flex flex-col items-center gap-2 text-center text-xs text-muted-foreground mt-2">
              {mode === "signup" ? (
                <p>
